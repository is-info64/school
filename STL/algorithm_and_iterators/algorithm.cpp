...
int main() {
	...
	std::cout << std::count_if(arr, arr + N, [](int a) { return a > 0; }) << std::endl; // 7
	...
	int c = 5, d = 6;
	std::swap(c, d);
	std::cout << c << d << std::endl; // 65
	
	// 7 accumulate (operation_func) sum all elements of array
	std::accumulate(arr, arr + N, 0, [](int acc, int el) {
		return acc + el;
	});

	// 8 Sort (comparator) Ascending
	std::sort(arr, arr + N, [](int curr, int next) {return curr < next; });
	std::for_each<int *>(arr, arr + N, [](const int& a) {
		std::cout << a << " ";
	});
